...
if uploaded_file is True or uploaded_file:  # Check for both boolean and file object
    ...
    try:
        ...
        if col_x not in df.columns or col_y not in df.columns or col_z not in df.columns or col_value not in df.columns:
            st.error(f"Certaines colonnes spécifiées n'existent pas dans le fichier CSV. Veuillez vérifier le mappage.")
            st.stop()
        ...
        for _, row in filtered_df.iterrows():
            if pd.notnull(row[col_x]) and pd.notnull(row[col_y]) and pd.notnull(row[col_z]) and pd.notnull(row[col_value]):
                ...
                if density_method == "Variable (colonne)" and density_column and density_column in row and pd.notnull(row[density_column]):
        ...
        if not composites_data:
            st.error("Aucun échantillon valide après filtrage. Veuillez vérifier vos filtres et mappages de colonnes.")
            st.stop()
    except Exception as e:
